...
**Current version:** v0.3 (Phase 3 complete, tag `phase-3-complete`, 2026-05-20). Not v1.0 — v1.0 is when Phase 5 pilots validate the system with real firms on real engagements with real deliverables shipped to real partners. We are not there yet.
...
**Phase 4 starting (Weeks 15-19):** human-in-the-loop / collaboration overhaul — review/approval workflow (consultant drafts → partner reviews → approve/request-changes), inline commenting threaded on claim_ids (comments survive memo edits because they're attached to ids, not text), engagement collaboration state surface, notification system, version history UI building on the W9 deepening versioning. The multi-user/multi-engagement spine exists from Phase 2 + W14/D3 sample workspace; Phase 4 is the workflow layer on top. Detailed scope at [docs/roadmap/phase4_scope.md](docs/roadmap/phase4_scope.md).

**Phase 5 (later, v1.0 gate):** real-firm pilots, Companies House TIFF/OCR, multi-instance cache invalidation, multi-language templates, Bloomberg / FactSet / Refinitiv connectors, mobile review experience, public sample workspace for prospective firms.

**Honest open items:** the growth_strategy Porter's content lands via the live LLM pipeline as `null` (gpt-4o legitimately emits Optional fields as null; the Phase 3 close fixed writer-truncation but the schema-enforcement work for non-nullable framework fields ships in Phase 4 Week 15). The W14/D3 sample workspace ships a hand-curated Porter's payload so the end-state is visible to demos; live production will catch up after the W15 schema work. PDF artifact rendering requires WeasyPrint's native libs (pango/cairo/gdk-pixbuf), which install cleanly in Docker but not on Windows dev hosts — the seeder + regression handle this with a graceful `skipped_no_weasyprint` status. No real-firm pilots have run yet; no real client deliverables have shipped through Argus end-to-end. v0.3, not v1.0.
