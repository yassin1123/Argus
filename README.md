...
**Pilot posture for the verifier (revised 2026-05-27, W22 Fix-Day):** **`ready` — pilot-ready as "verified" on the synthetic adversarial set, with real-claim production measurement the final confirmation step.** The W22/D5 posture (`human_review_required`) was set against numbers measured on the `heuristic_no_keys` fallback verifier — every prior W21/W22 calibration was heuristic-mode and underestimated the real engine. Once the gate-keeping pre-flight wired the actual cross-family ensemble (Claude + GPT-4o + DeBERTa-v3 + lexical, `verifier_source: cross_family_llm`), the synthetic FP-rate-on-supported dropped from 43.75% → **0.00%**, recall-on-insufficient rose to **100%**, accuracy to **75%**, and the W21/D4 red-team catch rate to **100%** (zero escapes — even the previously-documented `rt_007` misattribution and the W22/D3-introduced `rt_012` temporal-drift were caught). The "fully verified" trust claim is defensible at the synthetic stress-test level. Honest carve-out: the **real-claim production FP rate is still unmeasured** — Yassin's 30-60 min on the labelling CLI is the irreducible final step, and the production posture stays formally pending until that lands. The W22 Fix-Day wrap-up is at [docs/eval/week22_fix_day.md](docs/eval/week22_fix_day.md); the live numbers run on the W20 observability dashboard so quality stays monitored through the pilot.
